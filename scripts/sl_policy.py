...
from networks import policy_nn,policy_nn2
from utils import *
from env import Env
...
class SupervisedPolicy(object):
	...
	def __init__(self, learning_rate = 0.001):
		...
		with tf.variable_scope('supervised_policy', reuse=tf.AUTO_REUSE):
			...
			self.action_prob = policy_nn(self.state, state_dim, action_space, self.initializer,trainable=True)
			self.action_prob_old = policy_nn2(self.state, state_dim, action_space, self.initializer,trainable=True)
	...
	def update(self, state, action, alpha, sess = None):
		sess = sess or tf.get_default_session()
		_, loss = sess.run([self.train_op, self.loss], {self.state: state, self.action: action, self.tflam:alpha})
		print(loss)
		return loss
	...
	with tf.Session() as sess:
		...
		for episode in range(num_samples):
			...
			for item in good_episodes_test:
				...
				#a = open('test.txt', 'a+')
				#a.write(str(cos_sim)+'\n')
				policy_nn.update(state_batch, action_batch, math.exp(1-cos_sim))
