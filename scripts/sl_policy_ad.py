...
from networks_adapt import value_nn, value_nn_old, q_network
from utils import *
from env import Env
...
class SupervisedPolicy(object):
    ...
    def __init__(self, learning_rate=0.001):
        ...
        with tf.variable_scope('supervised_policy', reuse=tf.AUTO_REUSE):
            ...
            self.action_prob_adapt = tf.placeholder(tf.float32, [None], name='action_prob')
            self.value_nn = value_nn(self.state, self.action, state_dim, action_space, self.initializer)
            self.value_nn_old = value_nn_old(self.state, self.action, state_dim, action_space, self.initializer)
            ...
            self.crossE = tf.nn.softmax_cross_entropy_with_logits(labels=self.value_nn, logits=self.value_nn_old)
            ...
            self.action_mask = tf.cast(tf.one_hot(self.action, depth=action_space), tf.bool)
            self.picked_action_prob = tf.boolean_mask(self.action_prob, self.action_mask)
            self.loss = tf.reduce_sum(-tf.log(self.action_prob_adapt+0.1*self.picked_action_prob) + self.tflam * self.accr_subj_test) + sum(
    ...
    def predict_actionvalue(self, state, action, sess=None):
        sess = sess or tf.get_default_session()
        return sess.run(self.value_nn, {self.state: state, self.action: action})

    def update(self, state, action, alpha, action_prob, sess=None):
        sess = sess or tf.get_default_session()
        _, loss, kl, action, pap, padapt = sess.run(
            [self.train_op, self.loss, self.accr_subj_test, self.action, self.picked_action_prob,self.action_prob_adapt],
            {self.state: state, self.action: action, self.tflam: alpha, self.action_prob_adapt:action_prob})
    ...
    with tf.Session() as sess:
        ...
        for episode in range(num_samples):
            ...
            for item in good_episodes_test:
                ...
                # a = open('test.txt', 'a+')
                # a.write(str(cos_sim)+'\n')
                action_prob = [0]*len(action_batch)
                #action_value = policy_nn.predict_actionvalue(np.asarray([state_batch[0]]), np.asarray([action_batch[0]]))
                action_value = policy_nn.predict_actionvalue(state_batch,action_batch)
                for i in range(len(action_value)):
                    action_value_list.append(math.exp(abs(action_value[i])))
                    total_actionvalue +=math.exp(abs(action_value[i]))
                action_prob = [x / total_actionvalue for x in action_value_list]
                print('action_prob',action_prob)
                policy_nn.update(state_batch, action_batch, METHOD['lam'] * math.exp(1 - cos_sim), action_prob)
